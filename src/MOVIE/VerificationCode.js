...
const navigate = useNavigate()
  ...
  const [verificationCode, setVerificationCode] = useState(["", "", "", "", "", ""]); // Array to store six digits
  ...
  const handleVerifyCode = async () => {
    const joinedCode = verificationCode.join(""); // Join the array into a single string
    try {
      await checkActionCode(user, joinedCode); // Check if the code is valid
      await applyActionCode(user, joinedCode); // Apply the code to verify the email
